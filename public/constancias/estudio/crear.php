...
// Se obtienen y validan los parámetros de filtro (por ejemplo, vía GET)
$seccion = isset($_GET['seccion']) ? $_GET['seccion'] : '';
$grado   = isset($_GET['grado']) ? $_GET['grado'] : '';

// Se procesan las variables
$secc  = strtoupper(trim($seccion));
$grado = trim($grado);  // Suponemos que el nombre del grado es un string
...
<head>
  ...
  <!-- Se incluye Tailwind CSS (aquí se usa CDN, pero también puedes usar tu propia compilación) -->
        ...
        <tbody>
          ...
          if ($result) {
              while ($row = mysqli_fetch_assoc($result)) {
                  // Se escapan los datos para mostrarlos de forma segura
                  $id              = $row['id'];
                  ...
                  $cedularepre     = htmlspecialchars($row['cedularepre']);
                  $telefono        = htmlspecialchars($row['telefono']);
                  $correo          = htmlspecialchars($row['correo']);
                  $gradoNombre     = htmlspecialchars($row['grado_nombre']);
                  $seccionNombre   = htmlspecialchars($row['seccion_nombre']);
                  
                  echo '<tr class="border-b border-gray-900 dark:border-gray-700">';
                  echo '  <td class="px-3 py-2 bg-gray-200 dark:bg-gray-800">' . $nombre . '</td>';
                  echo '  <td class="px-3 py-2">' . $apellido . '</td>';
                  echo '  <td class="px-3 py-2 bg-gray-200 dark:bg-gray-800">' . $cen . '</td>';
                  echo '  <td class="px-3 py-2">' . $nacimiento . '</td>';
                  echo '  <td class="px-3 py-2 bg-gray-200 dark:bg-gray-800">' . $sexo . '</td>';
                  echo '  <td class="px-3 py-2">' . $representante . '</td>';
                  echo '  <td class="px-3 py-2 bg-gray-200 dark:bg-gray-800">' . $cedularepre . '</td>';
                  echo '  <td class="px-3 py-2">' . $telefono . '</td>';
                  echo '  <td class="px-3 py-2 bg-gray-200 dark:bg-gray-800">' . $correo . '</td>';
                  echo '  <td class="px-3 py-2">' . $gradoNombre . '</td>';
                  echo '  <td class="px-3 py-2 bg-gray-200 dark:bg-gray-800">' . $seccionNombre . '</td>';
                  echo '  <td class="px-3 py-2 bg-blue-400 dark:bg-blue-800 hover:bg-blue-500">';
                  echo '    <button class="w-full">';
                  echo '      <a href="imprimir.php?id=' . $id . '" class="px-3 py-2 block">';
                  echo '        <svg xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" viewBox="0 0 24 24" fill="none">';
                  echo '          <g id="File / Cloud_Upload">';
                  echo '            <path id="Vector" d="M12 16V10M12 10L9 12M12 10L15 12M23 15C23 12.7909 21.2091 11 19 11C18.9764 11 18.9532 11.0002 18.9297 11.0006C18.4447 7.60802 15.5267 5 12 5C9.20335 5 6.79019 6.64004 5.66895 9.01082C3.06206 9.18144 1 11.3498 1 13.9999C1 16.7613 3.23858 19.0001 6 19.0001L19 19C21.2091 19 23 17.2091 23 15Z" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>';
                  echo '          </g>';
                  echo '        </svg>';
                  echo '      </a>';
                  echo '    </button>';
              ...
              echo '<tr><td colspan="12" class="px-3 py-2 text-center">No se encontraron registros.</td></tr>';
          }
          ?>
        </tbody>
      </table>
