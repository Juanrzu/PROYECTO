...
// Notificaciones (igual que en el original)
...
// Función para actualizar el contador de caracteres (igual que en el original)
...
// Validación de campos (adaptada para recuperación)
function validarCampos() {
    ...
    if (!pregunta1.value.trim()) errores.push('Debe responder la primera pregunta de seguridad');
    if (!pregunta2.value.trim()) errores.push('Debe responder la segunda pregunta de seguridad');
    if (!nuevaContraseña.value.trim()) errores.push('Debe ingresar una nueva contraseña');
    ...
    // Validar longitud de nueva contraseña (8 caracteres mínimo)
    if (nuevaContraseña.value.trim() && nuevaContraseña.value.length < 8) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    updateCounter('p1', LIMITES.p1);
    updateCounter('p2', LIMITES.p2);
    updateCounter('contraseña', LIMITES.contraseña);
    
    // Event listeners para actualización en tiempo real
    usuario.addEventListener('input', () => updateCounter('usuario', LIMITES.usuario));
    pregunta1.addEventListener('input', () => updateCounter('p1', LIMITES.p1));
    pregunta2.addEventListener('input', () => updateCounter('p2', LIMITES.p2));
    nuevaContraseña.addEventListener('input', () => updateCounter('contraseña', LIMITES.contraseña));
});
...
form.addEventListener("submit", (e) => {
    ...
    if (errores.length > 0) {
        ...
        if (!usuario.value.trim() || !regexUsuario.test(usuario.value)) {
            usuario.focus();
        } else if (!pregunta1.value.trim()) {
            pregunta1.focus();
        } else if (!pregunta2.value.trim()) {
            pregunta2.focus();
        } else if (!nuevaContraseña.value.trim() || nuevaContraseña.value.length < 8) {
            nuevaContraseña.focus();
        }
    } else {
        // Si todo está correcto, mostrar mensaje de éxito
        notificaciones.mostrar('Contraseña cambiada con éxito', 'success');
    }
});
</script>
</body>
